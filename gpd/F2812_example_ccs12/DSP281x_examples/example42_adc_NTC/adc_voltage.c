...
// 功能描述:
//      用跳线帽将J9插件的3,4引脚短接
//    通过ADC通道A1采集模拟电压值，然后通过SPI传输给数码管
//    通过数码管显示电压值对应的温度值
********************************************************************/
...
#define ADC_CKPS   0x1   // ADC 模块时钟 = HSPCLK/1      = 25MHz/(1)     = 25MHz
#define ADC_SHCLK  0xf   // 采样窗口时间                 = 2 ADC 周期
#define AVG        100  // 平均采样限值
#define ZOFFSET    0x00  
#define BUF_SIZE   2048  // 采样缓冲器尺寸
...
// 步骤 4.初始化片内外设:
   ...
   AdcRegs.ADCCHSELSEQ1.bit.CONV00 = 0x1;   //ADC通道选择ADCIN1
...
{
register Uint16 i;
Uint16 *eptr=(Uint16 *)&TABLE[101]; //高端指针
Uint16 *sptr=(Uint16 *)TABLE; //低端指针
Uint16 *ptr; //查数指针
for(i=0;i<8;i++) //搜索全表
{
ptr = (Uint16 *)(sptr+((eptr-sptr)>>1));//数组的地址是连续的；
